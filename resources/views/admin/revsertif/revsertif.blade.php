        ...
        <div class="mx-auto max-w-screen-xl">
            <div class="mb-5 max-w-screen-xl rounded-lg bg-blue-600 px-5 py-8">
                <h1 class="mb-1 text-4xl font-extrabold tracking-tight text-white dark:text-white md:text-2xl lg:text-3xl">
                    Manajemen Revisi Sertifikat</h1>
                <p class="text-lg font-normal text-gray-50 dark:text-gray-400 lg:text-lg">Edit, download, atau hapus halaman
                    revisi sertifikat pada list dibawah ini</p>
            </div>
            <form class="mb-5 max-w-full" action="{{ route('searchrevsertifikat') }}" method="GET">
            ...
            {{-- @if (isset($search))
      <div id="alert-5" class="flex items-center p-4 border rounded-lg bg-gray-50 dark:bg-gray-800 mb-5" role="alert">
      ...
      @endif --}}
            ...
            <div class="relative mb-5 overflow-x-auto border-x border-t sm:rounded-lg">
                <table class="w-full text-left text-[15px] text-gray-500 dark:text-gray-400 rtl:text-right">
                    ...
                    @foreach ($revsertifikat->reverse() as $revsertif)
                        <tbody>
                            <tr
                                ...
                                <td class="px-6 py-4 font-medium text-gray-900">
                                    <a href="/formrevsertifikat/{{ $revsertif->slug }}">{{ $revsertif->judul }}</a>
                                </td>
                                <td class="px-6 py-4 md:whitespace-nowrap">
                                    <p>{{$revsertif->lasteditedby}}</p>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex flex-col justify-end gap-1 md:flex-row">
                                        ...
                                        <a href="{{ route('showsertifshow.revsertifikat', ['revsertif' => $revsertif->slug2]) }}"
                                        ...
                                        <a href="{{ route('adminrevsertifikat.slug', ['revsertif' => $revsertif->slug]) }}"
                                            class="rounded-lg bg-blue-600 px-3 py-2 text-center text-xs font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"><i
                                                class="ri-add-box-line"></i></a>
                                        <a href="{{ route('downloaddatarevsertifikat', ['id' => $revsertif->id]) }}"
                                            class="rounded-lg bg-blue-600 px-3 py-2 text-center text-xs font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"><i
                                                class="ri-download-line"></i></a>
                                        <a href="{{ route('editrevsertifikat', ['id' => $revsertif->id]) }}"
                                            class="rounded-lg bg-blue-600 px-3 py-2 text-center text-xs font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"><i
                                                class="ri-edit-box-line"></i></a>
                                        <a href="{{ route('hapusdatarevsertifikat', ['id' => $revsertif->id]) }}"
...
<div class="search-bar">
    <form action="{{ route('searchrevsertifikat') }}" method="GET">
...
<button><a href="{{ route('tambahrevsertifikat') }}">Tambah Pendaftaran</a></button>
...
@foreach ($revsertifikat as $revsertif)
    <div>
        <a href="/formrevsertifikat/{{ $revsertif->slug }}">{{ $revsertif->judul }}</a>

        <a href="{{ route('editrevsertifikat', ['id' => $revsertif->id]) }}">edit</a>
        <a href="{{ route('hapusdatarevsertifikat', ['id' => $revsertif->id]) }}">hapus</a>
        <a href="{{ route('downloaddatarevsertifikat', ['id' => $revsertif->id]) }}">download</a>
    </div>
    <br>
@endforeach --}}

...
@section('container')
    ...
    <div class="mx-auto max-w-[90%] md:max-w-main mt-6 mb-2 flex justify-between flex-col md:flex-row gap-7">
        <div class="md:w-[789px] w-full">
            <form class="flex gap-2 mb-5" action="{{ route('revsertifikat.search', ['revsertif' => $revsertif->slug2]) }}" method="GET">
                                    ...
                                    @foreach ($datas as $data)
                                        <tr>
                                            ...
                                            <td class="max-w-72 px-3 py-3 text-sm font-medium text-[#64748B]">
                                                ...
                                                <a href="{{ route('revsertifikat.user', ['revsertif' => $revsertif->slug2, 'datas' => $data->slug]) }}">{{ $data->nama }}</a></td>
                                            ...
                                            <td class="whitespace-nowrap px-3 py-3 text-sm font-bold text-[#1B7BE7]"><button
                                                    class="btn-fasilitas btn btn-warning"><a
                                                        href="{{ url($data->fasilitas)}}"
                                                        target="blank">Fasilitas</a></button></td>
                                            <td class="whitespace-nowrap px-3 py-3 text-sm font-bold text-[#1B7BE7]"><button
                                                    class="btn-sertif btn btn-warning"><a
                                                        href="{{ route('sertif.download02', ['id' => $data->id]) }}">Sertifikat</a></button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
